...
/*  클래스의 상속 - 자바에서 제공하는 클래스들의 상속관계를 이해하기 위함. 우리가 직접 만드는 것은몇가지 기능정도.
...
public class Shape {    
		...
		//도형의 너비가 MAX_WIDTH를 넘으면 도화지가 작아서 못그림... 등 기준을 정하는 예시
		//상수는 상속의 의미가 없습니다.

		
		// 인스턴스 필드
		// 일반적으로는 private 와 getter setter를 활용
		private String shapeName;		//도형 이름
		private int width;				//도형의 크기 중 너비
		private int height;				//도형의 크기 중 높이
		//접근한정자 테스트 할 필드
		protected String etc;			
		int round;						//접근한정자가 없는 경우 - default
		public double border;			//public은 어디서나 사용가능
