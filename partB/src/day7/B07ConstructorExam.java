...
public class B07ConstructorExam {
    public static void main(String[] args) {
        ...
        //문자열 객체 생성하기
        //1) 메소드 오버로딩 : 메소드 이름이 같으면서 리턴인자의 개수와 형식을 다르게 정의합니다.
        ...
        //                  message가 참조하는 "Hello~"과 동일한 리터럴을 temp가 참조할 때,
        //                 "Hello~"를 또 저장해서 temp가 참조하지 않고 이미 저장되어있는 "Hello~"를 참조
        System.out.println("message==temp ?"+ (message==temp));     //참조값 비교. 그러므로, message와 temp는 기억장소 동일 => true
        System.out.println("message==str2 ?"+ (message==str2));     // 단, new 연산으로 생성된 객체는 언제나 메모리를 새로 할당 => false
