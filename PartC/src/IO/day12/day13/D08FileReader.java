...
public class D08FileReader {
    ...
    public static void readByScanner(){
        ...
        //Scanner 클래스 : 1) 한줄씩 읽어오는 nextLine() 메소드, 2) 구분기호로 읽어오는 방법
    ...
    public static void readByChar(){
       // String filePath = "단어장.txt";
        String filePath = "C:\\Windows\\PFRO.log";          //517ms
        //문자열 저장하고 변경할 수 있는 가변객체
       ...
       try( FileReader fr = new FileReader(filePath)){
         ...
         while ((charValue = fr.read()) != -1) {        //read는 문자단위 입력. 문자를 1개씩 읽음.(입력)
            System.out.println((char)charValue);           //문장단위 이므로 표준출력가능
            // 파일에서 줄바꿈 2바이트 \r\n (10과13) -추측
    ...
    public static void readByCharArray(){
        //String filePath = "단어장.txt";
        String filePath = "C:\\Windows\\PFRO.log";          //29ms
        //StringBuilder content = new StringBuilder();
       ...
       try( FileReader fr = new FileReader(filePath)){
         ...
         while ((b = fr.read(charValues,0,256)) != -1) {        //read(배열이름,시작인덱스,최대문자길이)
            //읽은 문자의 개수는 리턴. b에 저장.
            System.out.print("읽은 내용 : " + new String(charValues,0,b)); 
    ...
    public static void readByCharBuffer(){
        String filePath = "C:\\Windows\\PFRO.log";          //25ms  한 줄이 256문자보다 많이 적으면 위의 char배열보다 시간은 더 소요될 수 있습니다.
