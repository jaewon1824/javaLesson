 ...
 * 
 * 			실제 비지니스 로직을 처리 하는 SQL이 많이 복잡해질 수 있습니다. (조인 횟수, 서브쿼리 횟수 많아짐)
 * 			=> 자주 사용하면서 일회용으로 처리가 되면 성능문제가 생깁니다. 비효율적.
 * 			=> 가상 테이블(뷰) 또는 프로시저(트랜잭션 단위로 구성)를 사용합니다.
 * 
 * 			View (뷰) : 가상 테이블
 * 						ㄴ 자주 사용하는 select 조회 결과를 미리 컴파일하여 오라클 객체로 저장.
 * 						ㄴ select 조회 할 때 테이블 처럼 사용. 그러나 insert update delete는 제약이 많고 사용되지 않음.
 * 			create view : 권한이 별도로 필요합니다.		=> 관리자 계정 sys 또는 system이 합니다.
 * 			1) SQL plus에서 grant create view to c##idev; 명령 실행하고
 * 			2) 디비버에서 접속 재연결
 */
...
-- 일반적인 테이블의 select 처럼 조회 형식 모두 사용 가능합니다.
SELECT * FROM V_CUSTOMSALE vc WHERE CUSTOMID = 'mina012';

SELECT * FROM V_CUSTOMSALE vc WHERE RANK =1;
