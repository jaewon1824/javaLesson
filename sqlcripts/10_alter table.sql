 ...
 * 참고 : 오라클에서는 테이블,컬럼명 등 식별자 이름에 쓸 수 있는 기호는 _ # $ 세가지 사용 가능
 */



CREATE TABLE tbl# (
   col1 varchar2(30) CONSTRAINT tbl#_notnull1 NOT NULL,			--CONSTRAINT 제약조건 이름 내용
   col2 varchar2(10) NOT NULL,  -- CONSTRAINT 키워드 생략 
   col3 varchar2(10),
   col4 number(3)
);
  -- CREATE 는 1번만 실행합니다. 오라클 객체 생성 이 후 변경사항은 ALTER 명령으로 한다.
...
-- 7) 제약조건 제거 : 제약조건은 변경은 없습니다. add 외 drop만 있습니다.
...
--alter table 로 col1을 primary key로 변경해보세요. (나중에는 alter table로 fk 추가도 할 수 있어야합니다.)
