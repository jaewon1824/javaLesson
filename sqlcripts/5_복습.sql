...
CREATE TABLE tbl_member (
	...
	join_date DATE default sysdate  	-- 가입날짜 , 날짜형식 : insert 명령은 날짜 패턴 문자열이 자동으로 날짜형식 변환
										-- default 는 기본값 키워드. sysydate는 현재날짜와 시간
...
-- `이메일이 naver 인 회원`
SELECT * FROM TBL_MEMBER WHERE EMAIL LIKE '%naver.com';
-- 6) 여러가지 값으로 조회 : `이름이 김모모 또는 박나연 또는 강나현`
SELECT * FROM TBL_MEMBER WHERE NAME IN ('김모모','박나연','강나현');
...
SEARCH * FROM TBL_MEMBER WHERE MNO BETWEEN 3 AND 6;
