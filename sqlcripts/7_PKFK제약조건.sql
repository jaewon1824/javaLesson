...
-- 2개의 테이블을 모두 삭제하려면 자식테이블 drop을 먼저 한 뒤 부모테이블 삭제가 가능합니다.
...
-- 2) 오류
INSERT INTO tbl_score(stuno,subject,jumsu,teacher,term)
VALUES ('2021001','영어',88,'김세리','2022_2');					--기본키를 구성하는 컬럼은 NULL 허용 못합니다.
...
SELECT * FROM TBL_SCORE ts WHERE STUNO = '2019019' AND SUBJECT ct ='영어';
SELECT * FROM TBL_SCORE ts WHERE STUNO = '2019019' AND SUBJECT ect ='역사';	-- 조회 결과 없음
...
- 성적테이블에 '과목명'을 학생 1명이 동일한 과목을 수강한다면(예를 들면 1학년 국어 ,  2학년 국어, 3학년 국어 )
  ...
  2) '과목'테이블 - 과목코드(PK),과목명(중복허용),담당교사과목명(중복허용)
  	 ...
  	 테이블을 구성하는 과정을 '정규화'.	정규화가 잘 되어있다 => 테이블 구성을 잘했다.
*/
