...
public class OracleConnectionTest {
    public static void main(String[] args) {
        ...
        String url = "jdbc:oracle:thin:@//localhost:1521/XE";       //localhost는 서버의 ip로 대체될 수 있다.
        String user = "c##idev";
        String password = "1234";
        try {
            // 2) ojdbc11.jar 라이브러리에 포함된 드라이버 클래스를 메모리에 로드
            Class.forName(driver);      //런타임 시점에 지정된 클래스명 로드    //자바6부터 드라이버매니저가 알아서 로드하기때문에 생략합니다.
            ...
            if(conn !=null){
                System.out.println("Connection 객체의 구현 클래스 : " + conn.getClass().getName()); //Connection 구현 클래스 이름 확인
                                                                        // oracle.jdbc.driver.T4CConnection 는 오라클 Connection 구현 클래스
